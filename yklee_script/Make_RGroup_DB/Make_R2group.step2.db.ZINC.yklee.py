...
if __name__ == "__main__":
    ...
    a = 0
    r2_list = itertools.combinations(R_dic.values(),2)
    for d in pkl_list:
        print(d)
        df = pd.read_pickle(d)
        for r2 in df['R2group']: #r2_list:
            temp_DB_work(df,r2,con_main)
    memoryDB2diskDB(con_main,'test.db')
    #sub_df = pd.DataFrame.from_dict(sub_dict,orient='index').reset_index().rename(columns={'index':'R2group',0:'ZID'})
    #sub_df.to_pickle('Except_R2.pickle')
    with open('Except_R2.pickle','wb') as F:
