...
def sql_connection_IDR():
    try:
        con = sqlite3.connect('./Data/DB_Table/ZINC_IDR.db')
        return con
    except Error:
        sys.exit(1)
        return
...
def SCF_result_analysis(k_set,mlist):
...
def SCF_result_analysis_core(li,k_set,mlist,Key_dics):
    t_set = Key_dics[li][0]
...
def checkmol_activation(afile, tmp_dic, con, zid_list, dup_zid_list,tmp_dic2):
...
if __name__ == "__main__":
    ...
    df_fin.to_csv('KR_result.csv',index=False)
