...
def Run_Babel(mylist,apair):
    ...
    for atoken in token:
        if 'Tanimoto from' in atoken:
            ...
            break
    token=re.split()
    #print(obabel_re,apair,token,token[-1])
    tani_on_off=float(token[-1])
    #print token[5]
    tmplist1.append(apair[0])
    tmplist1.append(apair[1])
    tmplist1.append(token[-1])
    #print tmplist1
...
def Clustering_ligand():
    ...
    ######################### YG  START #################################
    if int(sys.argv[3]) == 0:
        np_file='Input.save1.npy'
    elif int(sys.argv[3]) == 2:
        np_file='Input.save2.npy'
    ######################### YG END ####################################
    ...
    ############# YG START ############################################
    if int(sys.argv[3]) == 0:
        fw = open('l_cluster_DBSCAN1.txt', 'w')
        for ele in cluster_count_sorted:
        ...
        fw = open('l_cluster_DBSCAN2.txt', 'w')
        for ele in cluster_count_sorted:
            for e in cluster_list[ele[0]]:
                fw.write(e+'.sdf\n')
            fw.write('$$$\n')
