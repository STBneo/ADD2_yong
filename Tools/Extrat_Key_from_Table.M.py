...
def read_input_list():
    ...
    b_path='../Data/Table'
    input_file='Result_1.dat'
    
    #input_key=sys.argv[1]
    lines_1=[]
    lines_2=[]

    file_path=os.path.join(b_path,input_file)
    ...
    if os.path.isfile(file_path):
        ...
        while(True):
            ...
            if not line:
                ...
                if(f_7==1):
                    lines_2.append(token[1:])
                    #print '2:',token
                else:   
                    lines_1.append(token[1:])
                    #print '1:',token
    else:
        print 'There is no input file for key!'
        sys.exit(1)

    lines_1=sorted(lines_1, key=lambda e: int(e[0]),reverse=True )
    lines_2=sorted(lines_2, key=lambda e: int(e[0]),reverse=True )
    #print lines_1
    #print 
    #print lines_2
    #print
...
def parsing_cluster_line(line):
    ...
    for ele in line:
        ...
        token=ele.split(',')
        if len(token)>1:
            #print token[0][0:-1].strip(),int(token[1])
            tmp=[]
            tmp.append(token[0][0:-1].strip())
            tmp.append(int(token[1]))
            re_line.append(tmp)

    return re_line


def ch_round(key,tmp_set):
...
def M1():
    ...
    path_file=os.path.join(S_base,S_file)
...
def M2(Tani):
    ...
    for cluster in Tani:
        ...
        if int(cluster[0])>=3:
            #print cluster
            tmp_list=parsing_cluster_line(cluster)
            #print tmp_list
            for ele in tmp_list:
                #print ele[0]
                if(ch_round(ele[0],Key_set)==1):
                    Key_set.add(ele[0])
                else:
                    pass
    #print Key_set
...
def M3(Tani):
    ...
    for cluster in Tani:
        ...
        tmp_list=parsing_cluster_line(cluster)
        #print tmp_list[0][0]
        if(ch_round(tmp_list[0][0],Key_set)==1):
            Key_set.add(tmp_list[0][0])
        #print tmp_list
    #print Key_set
...
def M4(Tani):
    ...
    for cluster in Tani:
        ...
        #print tmp_list
        for ele in tmp_list:
            if not ele[0] in tmp_dic:
                tmp_dic[ele[0]]=int(ele[1])
            else:
                tmp_dic[ele[0]]+=int(ele[1])
   
    #print tmp_dic
    tmp_list=sorted(tmp_dic.items(),key=lambda x:x[1],reverse=True)
    #lines_1=sorted(tmp_dic.getitmes(), key=lambda e: int(e[0]),reverse=True )
...
def M5(Tani,DBSCAN):
    ...
    tmp_list_1=M4(Tani)
    tmp_list_2=M4(DBSCAN)
    tmp_list_1.update(tmp_list_2)
    #print tmp_list_1
...
def M6(Tani,DBSCAN):
    ...
    All=Tani+DBSCAN
    All=sorted(All, key=lambda e: int(e[0]),reverse=True )
...
def M9(Tani):
    ...
    Tani_tmp=[]
    tmp_dic={}
    Tani=sorted(Tani, key=lambda e: int(e[0]))

    #print Tani
    for cluster in Tani:
        if int(cluster[0])<=2:
            Tani_tmp.append(cluster)
    #print Tani_tmp

    for cluster in Tani_tmp:
        #print cluster
        tmp_list=parsing_cluster_line(cluster)
        #print tmp_list
        for ele in tmp_list:
...
def M10(DBSCAN):
    ...
    DBSCAN_tmp=[]
    tmp_dic={}
    DBSCAN=sorted(DBSCAN, key=lambda e: int(e[0]))

    #print Tani
    for cluster in DBSCAN:
        if int(cluster[0])<=2:
            DBSCAN_tmp.append(cluster)
    #print Tani_tmp

    for cluster in DBSCAN_tmp:
        #print cluster
        tmp_list=parsing_cluster_line(cluster)
        #print tmp_list
        for ele in tmp_list:
...
def Write_summary(akey,file_trace,ln_Input,Input_Dic,L_count):
...
def Sub_clustering(AFile_dic,akey,cid_list,L_count):
#def Sub_clustering(akey,cid_list):

    time_sp1=time()
    #Key_ln=len(sys.argv[1])
    Key_ln=len(akey)
    Top_rank=5
    ...
    #s_path='./Data/1D_Out/'
    s_path='./Data/1D_Out_'+str(L_count)+'R'
    ...
    Write_summary(akey,file_trace,ln_Input,Input_Dic,L_count)
...
def Sub_partition(AFile_dic,aline,l_count):
    ...
    for akey in in_keys:
        ...
        if(len(cid_lines)>=1):
            Sub_clustering(AFile_dic,akey,cid_lines,l_count)
        else:
            pass
...
def Scan_Only_Key(AFile_dic,Key_dic,aline,l_count):
    ...
    for akey in in_keys:
        ...
        #head_str='ZINC_ID,Main_Key_ID,Main_Key_freq,Sub_Key,Sub_Key_freq,Mw,AlogP,HBA Liipinski,HBD Lipinski,#RO5 Violations\n'
        ...
        Z_sdf_files=[fi for fi in pid_list  if fi.startswith('Z')]
        pid_list=Z_sdf_files
...
def main():
    ...
    T_base='../'
    T_file='input.key.m.list.txt'
    file_path=os.path.join(T_base,T_file)

    Tani,DBSCAN=lines=read_input_list()

    print '\nMaking a key list file for 1DScan......\n'

    Key_list.append(list(M1()))
    Key_list.append(list(M2(Tani)))
    Key_list.append(list(M3(Tani)))
    Key_list.append(list(M4(Tani)))
    Key_list.append(list(M5(Tani,DBSCAN)))
    Key_list.append(list(M6(Tani,DBSCAN)))
    Key_list.append(list(M9(Tani)))
    Key_list.append(list(M10(DBSCAN)))

    print Key_list

    fp_for_out=open(file_path,'w')

    for keys in Key_list:
        for akey in keys:
            fp_for_out.write(akey+' ')
        fp_for_out.write('\n')
    ...
    print '\nEnding generating a key file.'
    print 'The location of a key file is \'../input.key.m.list.txt\'.'
    print 'Processing the generating a key file: '+str(time2-time1)+'\n\n'


    #Scan_Only_Key(AFile_dic,Key_dic,aline,l_count)
    #Sub_partition(AFile_dic,aline,l_count)
