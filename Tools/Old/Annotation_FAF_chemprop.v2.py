...
def count(DB_path):
    DB_fname = 'ZINC_FAF.db'
    DB_name = 'FAF'
    try:
        con = sqlite3.connect(DB_path+DB_fname)
    except:
        print('Failed to connect '+DB+fname)
    con.text_factory = str
    cursorObj = con.cursor()
    cursorObj.execute('SELECT COUNT(*) FROM '+DB_name)
...
def write(path,DB_path,sdf_ls):
    ...
    fw.close()
    print(len(None_sdf_ls))
    #return None_sdf_ls

def sql_fetch_FAF(DB_path,zid):
    try:
        con = sqlite3.connect(DB_path+'ZINC_FAF.db')
    except:
        print('Failed to connect ZINC_FAF.db')
    con.text_factory = str
    cursorObj = con.cursor()
...
def main():
    ...
    #sdf_path = '../Data/Dic_Lib_Output_SDF/1R'
    #sdf_path = '../Data/Dic_Lib_Output_SDF/0R'
    sdf_ls = os.listdir(sdf_path)
    None_sdf_ls = write(path,DB_path,sdf_ls)
    #print(None_sdf_ls)
    #sys.exit(1)
