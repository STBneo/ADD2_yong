...
def make_sdf(score, l, pid):
    ...
    if score == str(0.7):
        ...
        R = sys.argv[2]
        #copy sdf files
        os.system('cp ../../Dic_Lib_Output_SDF/'+R+'/'+pid+'.sdf ./sdf')
...
def main():
    ...
    if score == str(0.7):
        ...
        fr = open('zid_list_'+R+'.txt', 'r')
        lines = fr.readlines()
        #print(lines)
        fw = open('zid_list_'+R+'_final.txt', 'w')
        for line in lines:
            line = line.strip()
            pid = line.split('.')[0]
            pid_ls.append(pid)
            if not os.path.isfile('../../Dic_Lib_Output_SDF/'+R+'/'+pid+'.sdf'):
                pid_ls.remove(pid)
        fr.close() 
        for pid in pid_ls:
            fw.write(pid+'\n')
