...
def Clustering_ligand():
    ...
    Input_path='../Data/l_cluster/l_cluster_0.7/not_clustered_sdf'
    ...
    if os.path.exists(path_file):
        ...
        print 'Making the input tanimoto matrix\n'
        for x_i in range(0,w):
            for y_i in range(0,h):
                # print x_i,y_i,tani_m[x_i][y_i]
                # babel l_IP4_CID_58809996_crystal_complex_2524.sdf  l_IP4_CHEMBL4103221_crystal_complex_0851.sdf  -ofpt
            
                #FNULL = open(os.devnull, 'w')
                #zp = subprocess.call(['7z', 'e', '-p=swshin', '-y', t_path+'/*.zip','-o'+t_path],stdout=FNULL,stderr=subprocess.STDOUT)

                obabel_re = subprocess.check_output(['obabel',l_dic[x_i],l_dic[y_i],'-ofpt'],stderr=subprocess.STDOUT)
                #print obabel_re
                token=obabel_re.split('\n')
                #tani_m_1[x_i][y_i]=1-float(token[6])
                #tani_m[x_i][y_i]=1-float(token[6])

                #tani_m[x_i][y_i]=float(token[6])
                #'''
                re=''
                for atoken in token:
                ...
                tani_m[x_i][y_i]=1-tani_on_off 

                '''
                print l_dic[x_i],x_i,l_dic[y_i],y_i,tani_on_off
                if tani_on_off > Cut_Off:
                    tani_m[x_i][y_i]=1.0
                else:
                    tani_m[x_i][y_i]=0.0
                ''' 
    ...
    for eps in EX_Range:
        ...
        cluster_cnt = len(set(Clustering.labels_))
        cluster_cnt_ls.append(cluster_cnt)
        print('eps : %f, the number of Cluster = %d'%(eps,len(set(Clustering.labels_))))
    print

    for eps in EX_Range:
        Clustering = DBSCAN(eps, min_samples,metric="precomputed").fit(tani_m)
        cluster_cnt = len(set(Clustering.labels_))
        if cluster_cnt == max(cluster_cnt_ls):
            print Clustering.labels_
            print 'The number of element :',len(Clustering.labels_)
            print 'The number of cluster :',len(set(Clustering.labels_))
            print 'The number of eps :', eps
            break
    ...
    fw = open('l_cluster_DBSCAN.txt', 'w')
    for ele in cluster_count_sorted:
        for e in cluster_list[ele[0]]:
            fw.write(e+'.sdf\n')
        fw.write('$$$\n')
    fw.close()
...
def main():
    ...
    #Scan_Only_Key(AFile_dic)
    #Sub_partition(AFile_dic)
    #Write_summary()
    ...
    test_data=np.load('amber_10k.npy')
    print test_data[0:2]
    print len(test_data)
    '''
