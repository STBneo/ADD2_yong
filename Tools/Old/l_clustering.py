...
def make_sdf(score, l, pid):
    ...
    if score == str(0.7):
        ...
        R = sys.argv[2]
        #copy sdf files
        fr = open('../../../Data/Dic_Lib_Output_SDF/'+R+'/'+pid+'.sdf', 'r')
        lines = fr.readlines()
        fw = open('./sdf/'+pid+'.sdf', 'w')
        for n, line in enumerate(lines):
            if n == 0: fw.write(pid+'\n')
        ...
        '''
        num = int(pid[5:8])
        fpath = '/lwork01/yg/ZINC/data/sdf_lib_merge_new/total'
        fname = 'ZINC'+str(num)+'.sdf'
        #print(fname)
        #print(pid)

        fw = open('./sdf/'+pid+'.sdf', 'w')
        with open(fpath+'/'+fname, 'r') as fr:
            for line in fr:
                if pid in line:
                    fw.write(line)
                    for line in fr:
...
def result_to_number(pid_ls, score):
    print('Change result to number result for preparing icliq')
    pid_dic = {}
...
def parsing_clustered(pid_ls, score):
    print('Change clustered.out.txt to chembl.clustered.out.txt by parsing')
    pid_dic = {}
...
def main():
    ...
    if score == str(0.7):
        ...
        R = sys.argv[2]
        print('chembl_list.'+R+'.txt')
        fr = open('chembl_list.'+R+'.uniq.txt', 'r')
        lines = fr.readlines()
        print(lines)
        fw = open('chembl_list.'+R+'.new.txt', 'w')
        for line in lines:
            line = line.strip()
            pid = line.split('.')[0]
            pid_ls.append(pid)
            if not os.path.isfile('../../../Data/Dic_Lib_Output_SDF/'+R+'/'+pid+'.sdf'):
                pid_ls.remove(pid)
        fr.close() 
        for pid in pid_ls:
            fw.write(pid+'\n')
