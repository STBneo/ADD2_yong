...
def F_Dir_Core(target_dirs,t_dir):

    # 
    #print target_dirs
    idx=target_dirs.index(t_dir)
    #print idx+1
    print 'processing 1D_Out_'+str(idx+1)+'R fold'
    ...
    arg='head -100000 '+path+'/All.f.final_re.'+token[-1]+'.csv > A.csv'
    os.system(arg)
...
def F_Dir():
    ...
    ln_target_dirs=len(target_dirs)
    #print ln_target_dirs
    #sys.exit(1)

    #n_cpu=multiprocessing.cpu_count()
    n_cpu=ln_target_dirs
    pool = multiprocessing.Pool(processes=1)
    #pool = multiprocessing.Pool(processes=n_cpu)
    func=partial(F_Dir_Core,target_dirs)
    pool.map(func,target_dirs)
    pool.close()
    pool.join()
