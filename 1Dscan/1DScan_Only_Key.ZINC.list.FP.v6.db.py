...
# filtering the file after saving version
...
def sql_connection_IDK():
    try:
        con = apsw.Connection('./Data/DB_Table/ZINC_IDK.db')
        return con
    except Error:
        print(Error)
        sys.exit(1)
...
def sql_search_IDK(con,Zid):
    cursorObj = con.cursor()
    cursorObj.execute('SELECT Zid, keys FROM IDK_Table where Zid=?',(Zid,))
    rows = cursorObj.fetchall()
    if len(rows)==0:
        return 0
    else:
...
def read_input_list():
    ...
    file_path=os.path.join(b_path,input_key)
    if os.path.isfile(file_path):
        fp_for_input=open(file_path,'r')
        while(True):
            line=fp_for_input.readline()
            if not line:
                break
            else:
                token=line.split()
...
def Make_dic_input(Sub_top5_keys):
    ...
    for akey in Sub_top5_keys:
        for af in f_list:
            af_f=sql_search_SF(con_SF,af)
            #print af, af_f
            if akey in af_f:
...
def Make_input_key_set():
    ...
    for af in f_list:
        af_f=sql_search_SF(con_SF,af)
        ln_feat=len(af_f)
        if(Min_Key_Size<=ln_feat):
            for s_i in range(0,ln_feat-Key_size+1):
                zkey=af_f[s_i:s_i+Key_size]
                if not zkey in dic_count:
                    #dic_id[zkey]=[acid]
                    dic_count[zkey]=1
                else:
                    #dic_id[zkey].append(acid)
                    dic_count[zkey]+=1
                #input_key_set.add(zkey)
    ...
    key_num=len(dic_count_sorted)
    #print key_num
    #Seed_Top_rank=int(float(key_num)*(float(Seed_Top_P)/float(100)))
    #print Seed_Top_rank

    #print dic_count_sorted[:Seed_Top_rank]
    for ele in dic_count_sorted[:Seed_Top_rank]:
        #print ele[0]
        input_key_set.add(ele[0])
    #sys.exit(1)
...
def Write_summary(akey,dic_count,Zid_set,ln_Input,Input_Dic,L_count):

    con_IDK=sql_connection_IDK()

    #O_base='./Data/1D_Out'
    O_base='./Data/1D_Out_'+str(L_count)+'R'
    ...
    if akey in Input_Dic:
        main_freq=dic_count[akey]
    else:
        main_freq=0

    #filtered_cid_list=Zid_list.split(',')
    #cid_set= set(filtered_cid_list[:-1])
    #print cid_set
    #sys.exit(1)
    ...
    for line in lines_set:
        ...
        #print Zid
        if Zid in Zid_set:
            key_list=sql_search_IDK(con_IDK,Zid)
            t_key_list=key_list.split(',')
            #print t_key_list[:-1],Input_Dic.keys()
            #sys.exit(1)
            #key_list=file_trace[Zid]
            for key in t_key_list[:-1]:
                if key in Input_Dic:
...
def Sub_clustering(akey,cid_set,R_count):

    con_F=sql_connection_F()
    Key_ln=len(akey)
    Min_Key_Size=5
    Key_size=5
    Top_rank=5
    ...
    #input_key_set=Make_input_key_set()
    #print input_key_set
    #sys.exit(1)
    ...
    for acid in cid_set:
    #for acid in cid_list[:10000]:  #For test
        acid=acid.strip()
        feat=sql_search_F(con_F,acid)
        ln_feat=len(feat)
        #print('akey:',akey,acid,ln_feat,feat)
        #return
        if ln_feat!=0:
            if(Min_Key_Size<=ln_feat):
                for s_i in range(0,ln_feat-Key_size+1):
                    zkey=feat[s_i:s_i+Key_size]
                    if not zkey in dic_count:
                        dic_count[zkey]=1
                    else:
                        dic_count[zkey]+=1
    ...
    print ' ->done!'
    time_dic2=time()
    #print 'Processing the Dic: '+str(time_dic2-time_dic1)+'\n\n'
    time_sort1=time()
    print ' Start sorting...'
    dic_count_sorted=sorted(dic_count.items(),key=lambda x:x[1],reverse=True)
    print ' ->done!'
    time_sort2=time()
    #print 'Processing the sort: '+str(time_sort2-time_sort1)+'\n\n'

    s_path='./Data/1D_Out_'+str(R_count)+'R'
    s_path_file=os.path.join(s_path,akey)

    # for v3 version
    print ' Writing the sub_key result to sub_cluster....'
    ...
    #print(dic_count_sorted[:Top_rank])
    #return

    #for ele in dic_count_sorted:
    for ele in dic_count_sorted[:Top_rank]:
        ...
        for afile in cid_set:
        #for afile in cid_list[:10000]: #For test
            afile=afile.strip()
            re_value=sql_search_F(con_F,afile)
            if(re_value==0):
                pass
                #print afile
            else:
    ...
    msf_list=list(set(top5_f_list))
    print ' Making input dictionary...'
    ln_Input,Input_Dic=Make_dic_input(msf_list)
    print ' ->done!'
    print ' Writing the summary file...'
    Write_summary(akey,dic_count,Zid_set,ln_Input,Input_Dic,R_count)
    print ' ->done!'

    print '--------------------| Summary |------------------------'
    print 'Target Key : ',akey
    print 'key length : ', Key_ln
    print 'Num of ZINC id : ', len(cid_set)
...
def Sub_partition(akey_line,R_count):

    time_sum1=time()
    print_date_time()

    ln_akey_line=len(akey_line)
    #print ln_akey_line
    n_cpu=multiprocessing.cpu_count()
    if(ln_akey_line<n_cpu):
        n_cpu=ln_akey_line
    else:
        n_cpu=n_cpu-1
    #print n_cpu
    ...
    func=partial(Sub_partition_core,akey_line,R_count)
    pool.map(func,akey_line)
    pool.close()
    pool.join()

    time_sum2=time()
    #print 'Total saved Zids in Summary : '+str(sl_count)
    print '**--------------------------------------------------------------------------------**'
    print '**    Ending the Sub_clustering, writing the result for Round '+str(R_count)+': '+str(time_sum2-time_sum1)+'   **'
    print '**--------------------------------------------------------------------------------**'
    return


def Sub_partition_core(akey_line,R_count,akey):

    O_base='./Data/1D_Out_'+str(R_count)+'R'
    #in_keys=akey_line
    
    key_count=akey_line.index(akey)
    ln_in_keys=len(akey_line)

    print '\n-- '+str(key_count+1)+'/'+str(ln_in_keys)+', '+str(R_count)+'-Round, Starting sub_clustering and writing the result for a key: '+akey+' --'
    print_date_time()
    ...
    fp_for_in.close()
    cid_set=set(cid_lines)

    #print('cid_set: ',cid_set)
    #return

    if(len(cid_set)>=1):
        Sub_clustering(akey,cid_set,R_count)
    else:
        pass
...
def Scan_Only_Key(akey_line,l_count):
    ...
    pool = multiprocessing.Pool(processes=n_cpu)
    func=partial(Scan_Only_Key_core,akey_line,l_count)
    pool.map(func,akey_line)
    pool.close()
    pool.join()
    ...
    print '\n**--------------------------------------------------------------------------------**'
    #print 'Total saved Zids in Summary : '+str(sl_count)
    print '**          Writing the summary 1DScan result for Round '+str(l_count)+': '+str(time_sum2-time_sum1)+'          **'
    print '**--------------------------------------------------------------------------------**'

    return


def Scan_Only_Key_core(akey_line,l_count,akey):


    O_base='./Data/1D_Out_'+str(l_count)+'R'
    in_keys=akey_line
    # for dic of seed sdf for filtering
    input_key_set=Make_input_key_set()
    #return


    # Making Out_put folder
    '''
    if(os.path.isdir(O_base)):
        arg='rm -r '+O_base+'/*'
        os.system(arg)
    else:
        arg='mkdir '+O_base
    ...
    print '\n-- '+str(key_count+1)+'/'+str(ln_in_keys)+', '+str(l_count)+'-Round, Starting 1/2D-scan for a key: '+akey+' --'
    #sys.exit(1)
    #for CSV
    #return
    time_cid1=time()

    fp_for_summary=open(O_base+'/Summary.'+akey+'.csv','w')
    head_str='ZINC_ID,Main_Key_ID,Main_Key_freq,Sub_Key,Sub_Key_freq\n'
    fp_for_summary.write(head_str)

    print_date_time()
    con_K=sql_connection_K()
    print ' Key searching in ZINC15 DB: '+akey+'.......'
    pid_str=sql_search_K(con_K,akey)
    print ' ->Done!'
    ...
    ##################################################################################################################################################
    # for a test 
    #ln_pid_list=len(pid_list)
    #pid_set=set(pid_list[:1000])
    #pid_set=set(pid_list[:500])
    #print 'Hi_New',akey,len(pid_list),ln_pid_list 
    #return
    ################################
    ...
    #print_date_time()
    print ' Writing the summary and Cid_list file for '+akey
    ll_count=0
    sl_count=0
    for afile in pid_set:
        feat_str=sql_search_IDK(con_IDK,afile)
        feat_list=feat_str.split(',')
        #return
        feat_set=set(feat_list[:-1])
        if unicode(akey,"utf-8") not in feat_set:
            continue
        re_set=feat_set&input_key_set
        '''
        print akey,afile,len(feat_list)
        print 'feat_list:',feat_list
        print 'input_key_set:',input_key_set
        print 're_set:',re_set
        print
        '''
        if(len(re_set)>0):
            ...
            tmp_str=''
            sl_count+=1
            # For Cid_list writing
            #f_name=afile+'.sdf'
            fp_for_Mkey_out.write(afile+'\n')
            ll_count+=1
    fp_for_Mkey_out.close()
    print ' ->Done!'
    fp_for_summary.close()
    time_cid2=time()
    print ' Total ths saved ZINC IDs: '+str(ll_count)
    #print '-- Ending the 1/2D-Scan for a key: '+akey+' and the excuation time: '+str(time_cid2-time_cid1)+' --'
...
def main():
    ...
    print '\n\n\n-------------------------------------'
    print '###                               ###'
    print '###      Staring 1/2D-Scan        ###'
    print '###                               ###'
    print '-------------------------------------\n'

    print '# Connceting the DB.......   '
    #con_F=sql_connection_F()
    #con_K=sql_connection_K()
    print '# Sucess connecting ZINC15 DB !!'
    key_lines=read_input_list()
    ln_key=len(key_lines[0][0])
    print '# The Key length of input is',ln_key,'\n' 
    #print key_lines
    #sys.exit(1)
   
    arg='rm -r ./Data/1D_Out_[1-8]R'
    os.system(arg)

    R_count=1
    for akey_line in key_lines:
        ...
        #print akey_line
        #sys.exit(1)
        Scan_Only_Key(akey_line,R_count)
        Sub_partition(akey_line,R_count)
        R_count+=1
    
    time_sum2=time()
    print '\n\n\n-- Total excution time: '+str(time_sum2-time_sum1)+' --'
